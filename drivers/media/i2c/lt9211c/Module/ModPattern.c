...
void Mod_DesscPll_ForPtn(struct ChipRxVidTiming *ptn_timing)
{
    ...
    LTLog(LT_LOG_INFO,"pclk_khz: %ld",pclk_khz);
...
void Mod_VidChkDebug_ForPtn(void)
{
    ...
    LTLog(LT_LOG_INFO,"ucframerate: %d",ucframerate);
	HDMI_WriteI2C_Byte(0xff,0x86);
    ...
    else if ((ushact == pattern_1920x1080_60Hz.hact ) &&(usvact == pattern_1920x1080_60Hz.vact ))
    {
        ...
        {
            LTLog(LT_LOG_INFO,"Video Pattern Set 1920x1080_90Hz");
        }
        else if (ucframerate > (pattern_1920x1080_100Hz.ucFrameRate - 3) && ucframerate < (pattern_1920x1080_100Hz.ucFrameRate + 3))
        {
            LTLog(LT_LOG_INFO,"Video Pattern Set 1920x1080_100Hz");
        }
        else
        {
            LTLog(LT_LOG_INFO,"Video Pattern Set 1920x1080_60Hz");
        }
    }
    else if ((ushact == pattern_2560x1440_60Hz.hact ) &&(usvact == pattern_2560x1440_60Hz.vact ))
    {
        LTLog(LT_LOG_INFO,"Video Pattern Set 2560x1440_60Hz");
    }
    else if ((ushact == pattern_3840x2160_30Hz.hact ) &&(usvact == pattern_3840x2160_30Hz.vact ))
    {
        LTLog(LT_LOG_INFO,"Video Pattern Set 3840x2160_30Hz");
    }
    else 
    {
        LTLog(LT_LOG_INFO,"No Video Set");
    }

    LTLog(LT_LOG_INFO,"hfp, hs, hbp, hact, htotal = %d %d %d %d %d",ushfp, ushs, ushbp, ushact, ushtotal);
	LTLog(LT_LOG_INFO,"vfp, vs, vbp, vact, vtotal = %d %d %d %d %d",usvfp, usvs, usvbp, usvact, usvtotal);
}
...
void Mod_MipiTxpll_ForPtn(struct ChipRxVidTiming *ptn_timing)
{
    ...
    LTLog(LT_LOG_DEBUG, "ulMipiDataRate: %ld, ulMpiTXPhyClk: %ld, ucSericlkDiv: %d, ucPreDiv: %d, ucDivSet: %d",
    ...
    {
        LTLog(LT_LOG_INFO,"LT9211C tx pll lock");
    }
    else
    {
        LTLog(LT_LOG_INFO,"LT9211C tx pll unlocked");
    }
...
void Mod_MipiTx_PhyTimingParaSet_ForPtn(void)
{
    ...
    LTLog(LT_LOG_DEBUG, "ulMipiDataRate: %ld, ulrdbyteclk: %ld",ulMipiDataRate, ulrdbyteclk);
    ...
    LTLog(LT_LOG_DEBUG, "ck_post (0xD4A9) = 0x%02x", ucClkPost);
    LTLog(LT_LOG_DEBUG, "ck_zero (0xD4A7) = 0x%02x", ucClkZero);
    LTLog(LT_LOG_DEBUG, "hs_lpx  (0xD4A4) = 0x%02x", ucHsLpx);
    LTLog(LT_LOG_DEBUG, "hs_prep (0xD4A5) = 0x%02x", ucHsPrep);
    LTLog(LT_LOG_DEBUG, "hs_trail(0xD4A6) = 0x%02x", ucHsTrail);
    LTLog(LT_LOG_DEBUG, "hs_rqst (0xD48A) = 0x%02x", ucHsRqStPre);
...
void Mod_MipiTxDig_Set_ForPtn(void)
{
   ...
   HDMI_WriteI2C_Byte(0x82,0x00);
    ...
    LTLog(LT_LOG_DEBUG,"readdly = 0x%02x%02x",(HDMI_ReadI2C_Byte(0x82)), (HDMI_ReadI2C_Byte(0x83)));
        ...
        LTLog(LT_LOG_INFO, "Mipi DSI Out");
    #else
        ...
        LTLog(LT_LOG_INFO, "Mipi CSI Out");
    #endif
}

void Mod_MipiTxPattern_Out(void)
{
    LTLog(LT_LOG_INFO,"*************LT9211C MIPITX Pattern Config*************");
    Mod_SystemInt();
    ...
    LTLog(LT_LOG_INFO,"*************LT9211C MIPITX Vid Pattern Out************");
    while(1);
}
...
void Mod_LvdsTxpll_ForPtn(struct ChipRxVidTiming *ptn_timing)
{
    ...
    u8 ucDivSet = 0;
    u8 temp = 1;
    //float ucPixClkDiv = 0;        // ARMv7l不支持浮点运算, 此浮点数用下面两个整数相除代替
    u8 ucPixClkDivNumerator;        // ucPixClkDiv的分子
    u8 ucPixClkDivIntDenominator;   // ucPixClkDiv的分母
...
//        ulLvdsTXPhyClk = ulDessc_Pix_Clk * 3.5;
    ...
    if (pclk_khz > 150000)
    {
        ...
        //ucPixClkDiv = 3.5;
        ucPixClkDivNumerator = 7;
        ucPixClkDivIntDenominator = 2;
    }
    else
    {
        // ucPixClkDiv = (u8)((ulLvdsTXPhyClk * ucSericlkDiv) / (pclk_khz * 7));
        /* ARMv7l不支持浮点运算，修改此else分支中和ucPixClkDiv的代码以规避，修改前后代码功能等价 */
        //ucPixClkDiv = (u8)(ucSericlkDiv/temp);//modify 2013.11.13
        ucPixClkDivNumerator = ucSericlkDiv;
        ucPixClkDivIntDenominator = temp;

        if (ucSericlkDiv <= temp) //(ucPixClkDiv <= 1)
        {
            HDMI_WriteI2C_Byte(0x33,0x00); //pixclk div sel /7
        }
        else if (ucSericlkDiv <= (temp * 2)) //((ucPixClkDiv > 1) && (ucPixClkDiv <= 2))
        {
            HDMI_WriteI2C_Byte(0x33,0x01); //pixclk div sel /14
        }
        else if (ucSericlkDiv <= (temp * 4)) //((ucPixClkDiv > 2) && (ucPixClkDiv <= 4))
        {
            HDMI_WriteI2C_Byte(0x33,0x02); //pixclk div sel /28
        }
        else if (ucSericlkDiv <= (temp * 8)) //((ucPixClkDiv > 4) && (ucPixClkDiv <= 8))
        {
   ...
   // ucDivSet = (u8)((ulLvdsTXPhyClk * ucSericlkDiv) / (pclk_khz / ucPreDiv));
    ...
    HDMI_WriteI2C_Byte(0x35,ucDivSet);
    LTLog(LT_LOG_DEBUG,"ulPclk_Khz: %ld, ucPreDiv: %d, ucSericlkDiv: %d, ucPixClkDiv: %u/%u, ucDivSet: %d",
                    pclk_khz, ucPreDiv, ucSericlkDiv, ucPixClkDivNumerator, ucPixClkDivIntDenominator, ucDivSet);

    LTLog(LT_LOG_INFO,"temp is: 0x%02x", temp);
    ...
    HDMI_WriteI2C_Byte(0x19,0xd4); //ssc_prd
        ...
        LTLog(LT_LOG_INFO,"LVDS Output SSC 1920x1080 30k2%");
        ...
        HDMI_WriteI2C_Byte(0x20,0x00);
        HDMI_WriteI2C_Byte(0x21,0x00);
        LTLog(LT_LOG_INFO,"LVDS Output SSC 3840x2160 30k2%");
    #endif
    ...
    HDMI_WriteI2C_Byte(0xff,0x85); 
        ...
        HDMI_WriteI2C_Byte(0xff,0x85);
            ...
            HDMI_WriteI2C_Byte(0x6f,(HDMI_ReadI2C_Byte(0x6f) | 0x04));
                ...
                LTLog(LT_LOG_INFO,"LvdsLaneNum: 5Lane");
        ...
        #else
                LTLog(LT_LOG_INFO,"LvdsLaneNum: 4Lane");
...
#if (LVDSTX_SYNC_INTER_MODE == ENABLED)
        LTLog(LT_LOG_INFO,"Lvds Sync Code Mode: Internal"); //internal sync code mode
    	HDMI_WriteI2C_Byte(0x68,(HDMI_ReadI2C_Byte(0x68) | 0x01));
    #if (LVDSTX_VIDEO_FORMAT == I_FORMAT)
            LTLog(LT_LOG_INFO,"Lvds Video Format: interlaced"); //internal sync code mode
            HDMI_WriteI2C_Byte(0xff,0x86);
            ...
            LTLog(LT_LOG_INFO,"Lvds Sync Code Send: respectively."); //sync code send method sel respectively
...
void Mod_LvdsTxPort_Swap_Forptn(void)
{
            ...
            LTLog(LT_LOG_INFO,"2Port Lvds Port Swap");
        #endif
...
{
    LTLog(LT_LOG_INFO,"*************LT9211C LVDSTX Pattern Config*************");
    Mod_SystemInt();
	...
	Mod_VidChkDebug_ForPtn();
    LTLog(LT_LOG_INFO,"*************LT9211C LVDSTX Vid Pattern Out************");
	while(1);
}
...
void Mod_TtlTxPhy_ForPtn(void)
{
    ...
    HDMI_WriteI2C_Byte(0x6D,0xff);
            ...
            LTLog(LT_LOG_INFO,"DDR Half Clock Mode:    Enable");
        #endif
            ...
            HDMI_WriteI2C_Byte(0x63,0x2b); //dll release
            HDMI_WriteI2C_Byte(0x63,0x2f); //phdly_en on
            LTLog(LT_LOG_INFO,"DDR Half Width Mode:    Enable");
        #endif
    #else
        ...
        HDMI_WriteI2C_Byte(0x2b,HDMI_ReadI2C_Byte(0x2b) | 0x01); //dessc double pix clock output
        LTLog(LT_LOG_INFO,"Half Bit Mode:    Enable");
    #endif
}
...
void Mod_TtlTxDig_ForPtn(void)
{
    ...
    HDMI_WriteI2C_Byte(0xaf,0x3f);
            ...
            LTLog(LT_LOG_INFO,"Video Format:    I Format");
        #else
            ...
            LTLog(LT_LOG_INFO,"Video Format:    P Format");
        #endif
    #endif
            ...
            LTLog(LT_LOG_INFO,"DDR Trans Mode:    High 12Bit First");
            #else
            HDMI_WriteI2C_Byte(0xaf,HDMI_ReadI2C_Byte(0xaf) & 0xbf); //low 12 Bit first
            LTLog(LT_LOG_INFO,"DDR Trans Mode:    Low 12Bit First");
            #endif
        #endif
    #endif
...
void Mod_TtlTxMapping_ForPtn(void)
{
    ...
    switch(TTLTX_OUTPUT_MODE)
    {
        case OUTPUT_RGB888:
            ...
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    RGB888");
            break;
        ...
        case OUTPUT_RGB666:
            ...
            //RGB666 swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x70);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    RGB666");
            break;
        ...
        case OUTPUT_RGB565:
            ...
            //RGB565 case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x70);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    RGB565");
            break;
        ...
        case OUTPUT_YCBCR444:
            ...
            //RGB high/low swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x74);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR444");
            break;
        
        case OUTPUT_YCBCR422_8BIT:
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_8BIT");
            break;
        
        case OUTPUT_YCBCR422_10BIT:
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_10BIT");
            break;
        
        case OUTPUT_YCBCR422_12BIT:
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_12BIT");
            break;
        ...
        case OUTPUT_YCBCR422_16BIT:
            ...
            //RGB high/low swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x72);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_16BIT");
            break;
        ...
        case OUTPUT_YCBCR422_20BIT:
            ...
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x40);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_20BIT");
            break;
        ...
        case OUTPUT_YCBCR422_24BIT:
            ...
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa8,0x40);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    YCBCR422_24BIT");
            break;
        ...
        case OUTPUT_BT656_8BIT:
            ...
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT656_8BIT");
            break;
        ...
        case OUTPUT_BT656_10BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT656_10BIT");
            break;
        ...
        case OUTPUT_BT656_12BIT:
            ...
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT656_12BIT");
            break;
        ...
        case OUTPUT_BT1120_8BIT:
            //RGB swap case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_8BIT");
            break;
        ...
        case OUTPUT_BT1120_10BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_10BIT");
            break;
        ...
        case OUTPUT_BT1120_12BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x01);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_12BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_16BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BTA_T1004_16BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_20BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BTA_T1004_20BIT");
            break;
        ...
        case OUTPUT_BTA_T1004_24BIT:
            //YC swap case 
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BTA_T1004_24BIT");
            break;
        ...
        case OUTPUT_BT1120_16BIT: 
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_16BIT");
            break;
        ...
        case OUTPUT_BT1120_20BIT:
            ...
            //other case
                ...
                HDMI_WriteI2C_Byte(0xa9,0x40);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_20BIT");
            break;
        ...
        case OUTPUT_BT1120_24BIT:
            //YC swap case 
                ...
                HDMI_WriteI2C_Byte(0xa9,0x00);
            #endif
            LTLog(LT_LOG_INFO,"TTLTx Output Mode:    BT1120_24BIT");
            break;
    }
}
...
void Mod_CscSet_ForTtlPtn(void)
{
        ...
        LTLog(LT_LOG_INFO,"Color Format:    RGB");
    #elif TTLTX_COLORSPACE == YUV444
        HDMI_WriteI2C_Byte(0xff,0x86);
        HDMI_WriteI2C_Byte(0x85,0x10);
        LTLog(LT_LOG_INFO,"Color Format:    YUV444");
        ...
        LTLog(LT_LOG_INFO,"Color Format:    YUV422");
    #endif
}
...
void Mod_TtlTxPhaseSet_ForPtn(void)
{
    ...
    LTLog(LT_LOG_INFO,"pix clk = %ld kHz",pclk_khz);
    LTLog(LT_LOG_INFO,"out clk = %ld kHz",io_dclk_khz);
    LTLog(LT_LOG_INFO,"DLL phase set: 0x%02x", ucphase);
    
}

void Mod_TtlTxPattern_Out(void)
{
    LTLog(LT_LOG_INFO,"*************LT9211C TTLTX Pattern Config*************");
    Mod_SystemInt();
    ...
    HDMI_WriteI2C_Byte(0x0d,0x7d); //ttl tx reset
    HDMI_WriteI2C_Byte(0x0d,0x7f); //ttl tx release
    LTLog(LT_LOG_INFO,"*************LT9211C TTLTX Vid Pattern Out************");
	while(1);
}
