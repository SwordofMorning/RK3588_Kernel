...
u8 Drv_VidChk_FrmRt_Get(void)
{
    ...
    /* ARMv7l不支持浮点除法，找不到makefile的gcc编译选项进行修改，先修改代码规避
     * 下面的代码等价于上面被注释掉的那条语句 */
    ucframerate = (u8)(25000000 / ulframetime);
    ulMod = 25000000 % ulframetime;
    if (ulMod * 2 >= ulframetime)
    {
        ucframerate += 1; // 余数>=除数的一半时进1，即第一位小数四舍五入
    }
