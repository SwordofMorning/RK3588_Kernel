...
void Drv_MipiRx_DesscPll_Set(void)
{
    ...
    LTLog(LT_LOG_INFO,"Mipi Rx PixClk: %ld",g_stRxVidTiming.ulPclk_Khz);
...
//        HDMI_WriteI2C_Byte(0x0a,0x42);
    ...
    do
    {
        ...
        ucPcr_Cal_Cnt++;
        LTLog(LT_LOG_WARN,"PCR unstable M = 0x%02x",(HDMI_ReadI2C_Byte(0x94)&0x7F));
    }while((ucPcr_Cal_Cnt < 10) && ((HDMI_ReadI2C_Byte(0x87) & 0x18) != 0x18));

    if((HDMI_ReadI2C_Byte(0x87) & 0x18) != 0x18)
    {
        LTLog(LT_LOG_INFO,"LT9211C pcr unstable");
        ucRtn = FAIL;
    }
...
void Drv_MipiRx_HsSettleSet(void)
{
    ...
    {
        LTLog(LT_LOG_DEBUG, "Set Mipi Rx Settle: 0x%02x", (g_stMipiRxVidTiming_Get.ucLane0SetNum - 5));
        HDMI_WriteI2C_Byte(0xff,0xd0);
        HDMI_WriteI2C_Byte(0x02,(g_stMipiRxVidTiming_Get.ucLane0SetNum - 5));
    }
    else
    {
        LTLog(LT_LOG_DEBUG, "Set Mipi Rx Settle: 0x03"); //mipi rx cts test need settle 0x0e
        HDMI_WriteI2C_Byte(0xff,0xd0);
...
void Drv_MipiRx_SotGet(void)
{
    ...
    LTLog(LT_LOG_DEBUG,"Sot Num = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SetNum,g_stMipiRxVidTiming_Get.ucLane1SetNum,
                                                                    g_stMipiRxVidTiming_Get.ucLane2SetNum,g_stMipiRxVidTiming_Get.ucLane3SetNum);
    LTLog(LT_LOG_DEBUG,"Sot Dta = 0x%02x, 0x%02x, 0x%02x, 0x%02x", g_stMipiRxVidTiming_Get.ucLane0SotData,g_stMipiRxVidTiming_Get.ucLane1SotData,
...
void Drv_MipiRx_HactGet(void)
{
    ...
    ulTemp = g_stMipiRxVidTiming_Get.uswc; // 消除浮点数

    switch (g_stMipiRxVidTiming_Get.ucFmt)
    {
        case 0x01: //DSI-YUV422-10bpc
        case 0x0e: //CSI-YUV422-10bpc
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 2 / 5; //g_stMipiRxVidTiming_Get.uswc / 2.5; //wc = hact * 20bpp/8
        break;
        ...
        break; 
        case 0x04: //RGB10bpc
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 4 / 15; //g_stMipiRxVidTiming_Get.uswc / 3.75; //wc = hact * 30bpp/8
        break;
        case 0x05: //RGB12bpc
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 2 / 9; //g_stMipiRxVidTiming_Get.uswc / 4.5; //wc = hact * 36bpp/8
        break;
        ...
        break;
        case 0x08: //RGB6bpc
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 4 / 9; //g_stMipiRxVidTiming_Get.uswc/ 2.25; //wc = hact * 18bpp/8
        break;
        ...
        break;
        case 0x0c: //RAW10
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 4 / 5; //g_stMipiRxVidTiming_Get.uswc / 1.25; //wc = hact * 10bpp/8
        break;
        case 0x0d: //RAW12
            g_stMipiRxVidTiming_Get.usHact = ulTemp * 2 / 3; //g_stMipiRxVidTiming_Get.uswc / 1.5; //wc = hact * 12bpp/8
        break;
        default: 
            g_stMipiRxVidTiming_Get.usHact = g_stMipiRxVidTiming_Get.uswc / 3; //wc = hact * 24bpp/8
        break;
...
u8 Drv_MipiRx_VidTiming_Get(void)
{
    ...
    {
        LTLog(LT_LOG_ERROR,"RX No Video Get");
        return FAIL;
    }
    else
    {
        LTLog(LT_LOG_INFO,"hact = %d",g_stMipiRxVidTiming_Get.usHact);
        LTLog(LT_LOG_INFO,"vact = %d",g_stMipiRxVidTiming_Get.usVact);    
        LTLog(LT_LOG_INFO,"fmt = 0x%02x", g_stMipiRxVidTiming_Get.ucFmt);
        LTLog(LT_LOG_INFO,"pa_lpn = 0x%02x", g_stMipiRxVidTiming_Get.ucPa_Lpn);
        return SUCCESS;
    }
}
...
{	
    ...
    for (uci = 0; uci < ucResolutionnum; uci++)
    {
            ...
            ( g_stMipiRxVidTiming_Get.usVact == resolution[uci].usVact ))
        {
            ...
            LTLog(LT_LOG_INFO,"FrameRate = %d", g_stMipiRxVidTiming_Get.ucFrameRate);
               ...
               (g_stMipiRxVidTiming_Get.ucFrameRate < (resolution[uci].ucFrameRate + 3)))
            {
                ...
                LTLog(LT_LOG_INFO,"Video Timing Set %dx%d_%d",g_stRxVidTiming.usHact,g_stRxVidTiming.usVact,g_stMipiRxVidTiming_Get.ucFrameRate);
                rtn = true;
                break;
            }
        }
...
u8 Drv_MipiRx_VidFmt_Get(IN u8 VidFmt)
{
    ...
    LTLog(LT_LOG_INFO,"MipiRx Input Format: %s",g_szStrRxFormat[VidFmt]);
    return ucRxVidFmt;
}
...
void Drv_MipiRx_InputSel(void)
{
    ...
    LTLog(LT_LOG_INFO,"Mipi CSI Input");
    #else 
    ...
    LTLog(LT_LOG_INFO,"Mipi DSI Input");
    #endif
}
...
void Drv_MipiRx_VidChkDebug(void)
{
    ...
    LTLog(LT_LOG_INFO,"mipi input hfp, hs, hbp, hact, htotal = %d %d %d %d %d",ushfp, ushs, ushbp, ushact, ushtotal);
	LTLog(LT_LOG_INFO,"mipi input vfp, vs, vbp, vact, vtotal = %d %d %d %d %d",usvfp, usvs, usvbp, usvact, usvtotal);
}
void Drv_MipiRx_PhyPowerOn(void)
{
    ...
    #endif
    LTLog(LT_LOG_INFO,"MIPI Input PortA");
    #elif MIPIRX_PORT_SEL == PORTB
    ...
    #endif
    LTLog(LT_LOG_INFO,"MIPI Input PortB");
    #else
